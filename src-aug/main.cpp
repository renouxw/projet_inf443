...
void initialize_data()
{
    ...
    image_raw const im3 = image_load_png("assets/squirrel.png");
    GLuint const texture_image_id3 = opengl_texture_to_gpu(im3,
            GL_MIRRORED_REPEAT /**GL_TEXTURE_WRAP_S*/,
            GL_MIRRORED_REPEAT /**GL_TEXTURE_WRAP_T*/);
    foliage.texture = texture_image_id3;
    ...
    tree_position = generate_positions_on_terrain(40);
    ...
    //--------------------------------------//
    // Key positions
    key_positions = { {10,10,5}, {9,10,5}, {8,10,5}, {7,10,5} };
    // Key times
    key_times = {0.0f, 0.5f, 1.0f, 1.5f};
    ...
    float const r = 0.03f; // radius of the sphere
...
void display_scene()
{
    ...
    // set the values for the spotlights (possibly varying in time)
        ...
        {
            scene.spotlight_color[k] = {0.8f, 0.8f, 0.75f};
    ...
    hierarchy["body"].transform.rotate = rotation({0,0,1}, -0.5*t + 0.4);
...
// Numerical integration
      //  if (p[2] < evaluate_terrain((p[0]/20)+0.5f, (p[1]/20)+0.5f).z + 0.05f){
        //    vec3 n = normale_terrain((p[0]/20)+0.5f, (p[1]/20)+0.5f) ;
          //  vec3 k = normalize(n);
          //  vec3 u = v - 2*dot(v,k)*k ;
           // v = vec3(0.9*(u.x), 0.9*(u.y), 0.9*(u.z)) ;
           // p[2] = evaluate_terrain((p[0]/20)+0.5f, (p[1]/20)+0.5f).z +0.06f;
        //}
        //else {
          //  v = v + dt*F/m;
            //p = p + dt*v;
//        }
        ...
        // Balles qui roulent sur le terrain
        //vec3 n = normalize(normale_terrain((p[0]/20)+0.5f, (p[1]/20)+0.5f)) ;
        //float f = std::sqrt(n.x*n.x + n.y*n.y);
        //p = evaluate_terrain((p[0]/20)+0.5f+ 0.0004f*f*n.x, (p[1]/20)+0.5f+ 0.0004f*f*n.y);
